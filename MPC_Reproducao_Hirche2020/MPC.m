...
% Reproducao do artigo Hirche2020
% Autor: Aline Isabel
% Data: 27/08/2026
clc;clear;
close all;
...
x{1}(:,1) = [pi/3;10]; x{2}(:,1) = [pi/10;0]; x{3}(:,1) = [pi/3;-5];
% x{4}(:,1) = [pi/4;-1];% x{5}(:,1) = [pi/(3.2);8];% x{6}(:,1) = [pi/2;15];% x{7}(:,1) = [0;0];
% x{8}(:,1) = [3*pi/2;-4]; % x{9}(:,1) = [3*pi/2;8];% x{10}(:,1) = [2*pi-0.3;3];% x{11}(:,1) = [pi/2;-15];
% x{12}(:,1) = [pi;0];% x{13}(:,1) = [pi;20];% x{14}(:,1) = [0;-17];% x{15}(:,1) = [pi/5;3];
% x{16}(:,1) = [pi/(7);8];% x{17}(:,1) = [pi/20;10];% x{18}(:,1) = [3*pi/5;10];% x{19}(:,1) = [3*pi/2;1];
% x{20}(:,1) = [pi-0.3;1];
...
controller = mpc_controller(A,B,C,N,Q,R,P,bo,Oq,Sf,na,nx,nu,ny,Su,bu,Tx,Tu,nt,Ay,by); % Cria o problema
...
for k = 1:kend
    ...
    for i = 1:na
        for j = 1+1:na
        yini{i} = yini{i} + ybar_opt{i}(:,k)+0.1*(ybar_opt{j}(:,k)-ybar_opt{i}(:,k));
        end
    end
...
stairs(t(1:end-1),xbar_opt{i}(1,:),'--','linewidth',1)
end
...
stairs(t(1:end-1),xbar_opt{i}(2,:),'--k','linewidth',1)
end
