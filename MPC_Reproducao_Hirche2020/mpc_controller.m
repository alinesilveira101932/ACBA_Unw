function controller = mpc_controller(A,B,C,N,Q,R,P,bo,Oq,Sf,na,nx,nu,ny,Su,bu,Tx,Tu,nt,Ay,by)
...
for j = 1:na
    ...
    constraints = [constraints, (Sf*Tx + Oq)*qbar{j} <= bo];
    constraints = [constraints, Ay*ybar{j} <= by];
end

% Acoplamento de saida
for i = 1:na
     constraints = [constraints, [(A-eye(nx)) B; C zeros(ny,nu)]*[xbar{i};ubar{i}] == [zeros(nx,ny); ybar{i}]];
     constraints = [constraints, [xbar{i};ubar{i}] == inv([(A-eye(nx)) B; C zeros(ny,nu)])*[zeros(nx,ny); ybar{i}]];
    for j = 1+i:na
            objective = objective + 1*(ybar{i}-ybar{j})'*(ybar{i}-ybar{j});
    end
end
